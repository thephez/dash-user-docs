...
Extract Dash Core as follows::
  ...
  tar xzf dashcore-2222.22.222-x86_64-linux-gnu.tar.gz

This will create a folder named ``dashcore-2222.22.222`` in the current working
directory. We will now install the executable binaries to
``/usr/local/bin`` using the ``install`` command::

  sudo install -m 0755 -o root -g root -t /usr/local/bin dashcore-2222.22.222/bin/*
